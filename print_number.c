...
int print_number_right_shift(char *str, params_t *parameters)
{
	...
	if (neg && i < parameters->width
			&& pad_char == '0' && !parameters->minus_flag)
		str++;
	else
		neg = 0;
	if ((parameters->plus_flag && !neg2) ||
			(!parameters->plus_flag &&
			 parameters->space_flag && !neg2))
		i++;
	if (neg && pad_char == '0')
		n += _putchar('-');
	if (parameters->plus_flag && !neg2
			&& pad_char == '0' && !parameters->unsign)
		n += _putchar('+');
	else if (!parameters->plus_flag && parameters->space_flag
			&& !neg2
	...
	if (neg && pad_char == ' ')
		n += _putchar('-');
	if (parameters->plus_flag && !neg && pad_char == ' '
			&& !parameters->unsign)
		n += _putchar('+');
	else if (!parameters->plus_flag && parameters->space_flag
			&& !neg2
...
int print_number_left_shift(char *str, params_t *parameters)
{
	...
	if (parameters->zero_flag && !parameters->minus_flag)
		pad_char = '0';
	neg = neg2 = (!parameters->unsign && *str == '-');
	if (neg && i < parameters->width && pad_char == '0'
			&& !parameters->minus_flag)
		str++;
	else
		neg = 0;
	if (parameters->plus_flag && !neg2 && !parameters->unsign)
		n += _putchar('+'), i++;
	else if (parameters->space_flag && !neg2
			&& !parameters->unsign)
		n += _putchar(' '), i++;
	n += _puts(str);
	while (i++ < parameters->width)
